...
-- Add some sample therapies data if not already present
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.therapies WHERE name = 'Panchakarma Detox') THEN
...
-- Add some sample time slots if not already present
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.time_slots WHERE start_time = '09:00') THEN
